...
@SpringBootTest
public class AliYunCodeTest {

    @Test
    public SendSmsResponse testAliYunCode() throws Exception {
        //设置超时时间-可自行调整
        ...
        SendSmsRequest request = new SendSmsRequest();
            ...
            request.setPhoneNumbers("13233144525,16636182256");
            //必填:短信签名-可在短信控制台中找到
            request.setSignName("赤行课堂");             //签名
            //必填:短信模板-可在短信控制台中找到，发送国际/港澳台消息时，请使用国际/港澳台短信模版
            request.setTemplateCode("SMS_205621548");   //模板id
            //可选:模板中的变量替换JSON串,如模板内容为"亲爱的${name},您的验证码为${code}"时,此处的值为
            //友情提示:如果JSON中需要带换行符,请参照标准的JSON协议对换行符的要求,比如短信内容中包含\r\n的情况在JSON中需要表示成\\r\\n,否则会导致JSON在服务端解析失败
            //参考：request.setTemplateParam("{\"变量1\":\"值1\",\"变量2\":\"值2\",\"变量3\":\"值3\"}")
            request.setTemplateParam("{\"code\":\"332789\"}");
    ...
    public void testsendCode(){
        try {
            SendSmsResponse sendSmsResponse = testAliYunCode();
    ...
    void  testA(){
        ...
        String s = AliYunUtil.sendPhoneMsg("13233144525", securityCode);
